<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Instrument Families</h2>
            <a href="{{ route('admin.instrument-families.create') }}"
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="divide-y divide-gray-100 text-sm text-gray-800">
                                @foreach ($instrumentFamilies as $instrumentFamily)
                                    <tr>
                                        <td class="py-3 pe-4">{{ $instrumentFamily->name }}</td>
                                        <td class="py-3 pe-4">{{ $instrumentFamily->slug }}</td>
                                        <td class="py-3 text-right">
                                            <div class="flex justify-end gap-2">
                                                <a href="{{ route('admin.instrument-families.edit', $instrumentFamily) }}"
                                                    class="text-indigo-600 hover:text-indigo-800">Edit</a>
                                                <form
                                                    action="{{ route('admin.instrument-families.destroy', $instrumentFamily) }}"
                                                    method="POST"
                                                    onsubmit="return confirm('Delete this instrument family?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
